...
def findLargestConnectedComponent(binary_image):
...
def cc_ransac(image, patch, indices_patch, depth, points_3d, s, t, Tc, T, N):
    ...
    for _ in range(N):
        ...
        # inliers = np.zeros((image.shape[0], image.shape[1], 1))
        inliers = np.zeros((points_3d.shape[0], points_3d.shape[1], 1))
        ...
        # labels, stats = findConnectedComponents(inliers)
        # max_label = np.argmax(stats[0:]) + 1
        # largest_cc = (labels == max_label).astype(np.uint8)
        largest_cc = findLargestConnectedComponent(inliers)
        size = np.count_nonzero(largest_cc == 255)

        # Step 4: Keep only large connected components
        if size >= Tc:
            inliers = np.column_stack(np.where(largest_cc))
            inliers_3d = points_3d[inliers[:, 0], inliers[:, 1]]

            # Step 5: Terminate if size of largest connected component is >= T
            if size >= T:
                return fit_plane(inliers_3d[:, 0], inliers_3d[:, 1], inliers_3d[:, 2])

            # Step 6: If the size of inliers is >= T, re-estimate the model
            if len(inliers) >= T:
                return fit_plane(inliers_3d[:, 0], inliers_3d[:, 1], inliers_3d[:, 2])
            ...
            if len(inliers) > max_inliers:
                ...
                best_model = fit_plane(inliers_3d[:, 0], inliers_3d[:, 1], inliers_3d[:, 2])

    # Step 7: Return the best model after N trials
    return best_model
